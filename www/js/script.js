...
function dettaglioEvento(idEvento) {
    ...
    if (idUtente) {
        ...
        const urlEndPV = new URL('https://localhost:5000/prenotazione');
...
function registraUtente() {
    ...
    fetch('https://localhost:5000/utente', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
function registraPrenotazione() {
    ...
    if (idPrenotazione == 0) {
        ...
        fetch('https://localhost:5000/prenotazione', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        .then(data => {
            if (data.message) {
                ...
                apriMessageBox(data.message + ". Il prezzo è di Euro " + data.prezzo + " da saldare in loco il giorno dell\'evento.");
        ...
        fetch(`https://localhost:5000/prenotazione/${idPrenotazione}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
